-- 0051: Premium V-Card backend — public card data + lead-capture inquiry.
-- Additive only. Two anon-callable SECURITY DEFINER RPCs:
--   promoter_card(code)  → public card payload for an APPROVED promoter
--   card_inquiry(...)    → visitor inquiry from the public card → routed lead
-- The verified badge stays admin-controlled: it reflects partner_status,
-- which only admin_review_partner can change.
...
begin
  ...
  -- live inventory the promoter can present (featured first, never sold)
...
revoke execute on function public.promoter_card(text) from public;
grant execute on function public.promoter_card(text) to anon, authenticated, service_role;

-- ── inquiry → routed lead + promoter notification ──────────────────────────
...
begin
  ...
  -- dedupe: same visitor mobile to the same promoter within 10 minutes
  if exists (
    select 1 from leads
...
revoke execute on function public.card_inquiry(text, text, text, text) from public;
grant execute on function public.card_inquiry(text, text, text, text) to anon, authenticated, service_role;
